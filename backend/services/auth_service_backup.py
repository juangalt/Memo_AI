...
class AuthService:
    ...
    def authenticate(self, username: str, password: str) -> Tuple[bool, Optional[str], Optional[str]]:
        ...
        try:
            ...
            logger.info(f"Unified authentication successful for user: {username} (admin: {user.is_admin})")
            return True, session_token, None
    ...
    def validate_unified_session(self, session_token: str) -> Tuple[bool, Optional[Dict[str, Any]], Optional[str]]:
        """
        Validate unified session (works for both users and admins)
        ...
        try:
            ...
            logger.error(f"Unified session validation failed: {e}")
            return False, None, f"Session validation error: {str(e)}"
...
def authenticate_unified(username: str, password: str) -> Tuple[bool, Optional[str], Optional[str]]:
    """
    Unified authentication for both users and admins
...
def validate_unified_session(session_token: str) -> Tuple[bool, Optional[Dict[str, Any]], Optional[str]]:
    """
    Validate unified session (works for both users and admins)
    ...
    """
    service = get_auth_service()
    return service.validate_unified_session(session_token)
